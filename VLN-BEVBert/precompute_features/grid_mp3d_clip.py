...
def get_patch_fts(model, img_transforms, device, images):
    ...
    patch_fts = torch.zeros((1,), device=device, dtype=model.dtype)
    patch_hook = model.visual.transformer.register_forward_hook(hook_builder(patch_fts))
    with torch.no_grad():
        global_fts = model.encode_image(images)
    patch_fts = patch_fts.permute(1,0,2)[:, 1:, :]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_file', default='img_features/vit_b16_224_clip_patch.hdf5')
